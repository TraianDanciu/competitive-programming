...
int canGetIntervals(int num_intervals, int maximum) {
  int i, cnt, ok = 1;
  for(i = 0; i <= 2 * n; i++) {
    maxop[i] = (maximum + num_intervals - v[i]) / 2;
    if(maximum + num_intervals < v[i]) {
      ok = 0;
    }
  }
  aint.init(2 * n + 1);
...
void reconstructSolution() {
  canGetIntervals(answerk, answer);
}
